...
service_name = "VLANChangerApp"  # Keyring service name
...
def save_settings():
    try:
        if remember_var.get() == 1:
            ...
            keyring.set_password(service_name, settings["username"], pass_entry.get())
    ...
    try:
        if os.path.exists(settings_file):
            with open(settings_file, "r") as f:
                ...
                saved_pass = keyring.get_password(service_name, settings.get("username", ""))
                if saved_pass:
...
def _get_interfaces():
    ...
    try:
        ...
        root.after(0, lambda: messagebox.showerror("Error", f"Failed to get interfaces: {e}"))
    finally:
        set_loading(False)
...
def change_port_mode():
    ...
    try:
        ...
        get_interfaces()  # Refresh interface list after mode change
    except Exception as e:
...
disconnect_btn = ttk.Button(frame_topbar, text="Disconnect", bootstyle="danger-outline",
                            command=disconnect_switch, state="disabled")
...
remember_check = ttk.Checkbutton(frame_conn, text="Remember Me",
                                 variable=remember_var, bootstyle="secondary")
remember_check.grid(row=6, column=0, sticky="w", pady=5)

ttk.Button(frame_conn, text="Connect to Switch", bootstyle="success-outline",
           command=connect_switch).grid(row=7, column=0, pady=8, sticky="ew")
...
frame_int.grid(row=1, column=0, sticky="nsew", pady=5)
frame_int.columnconfigure(0, weight=1)
frame_int.rowconfigure(0, weight=0)
frame_int.rowconfigure(1, weight=0)
frame_int.rowconfigure(2, weight=0)
frame_int.rowconfigure(3, weight=1)

interface_var = tk.StringVar()
interface_dropdown = Combobox(frame_int, textvariable=interface_var, state="disabled", font=("Segoe UI", 11))
interface_dropdown.grid(row=0, column=0, sticky="ewns", pady=5)
refresh_btn = ttk.Button(frame_int, text="🔄 Refresh Ports", bootstyle="info-outline", command=get_interfaces)
refresh_btn.grid(row=0, column=1, padx=5, pady=5, sticky="ewns")

port_mode_var = tk.StringVar()
frame_mode = ttk.Frame(frame_int)
frame_mode.grid(row=3, column=0, sticky="ewns", pady=5)
frame_mode.columnconfigure(0, weight=1)
ttk.Label(frame_mode, text="Port Mode:", font=("Segoe UI", 10, "bold")).pack(side="left")
ttk.Radiobutton(frame_mode, text="Access", variable=port_mode_var,
                value="access", command=change_port_mode).pack(side="left", padx=10)
ttk.Radiobutton(frame_mode, text="Trunk", variable=port_mode_var,
                value="trunk", command=change_port_mode).pack(side="left", padx=10)

ttk.Label(frame_int, text="Current VLAN:", font=("Segoe UI", 10, "bold")).grid(row=1, column=0, sticky="w", pady=3)
current_vlan = tk.StringVar()
current_vlan_label = ttk.Label(frame_int, textvariable=current_vlan,
                               font=("Segoe UI", 12, "bold"), bootstyle="warning", anchor="center")
current_vlan_label.grid(row=2, column=0, pady=5, sticky="ewns")
...
vlan_entry.grid(row=1, column=0, sticky="ewns", pady=5)

apply_vlan_btn = ttk.Button(frame_vlan, text="Apply VLAN Change",
                            bootstyle="danger-outline", command=change_vlan, state="disabled")
apply_vlan_btn.grid(row=2, column=0, pady=8, sticky="ewns")

# Loading indicator
loading_var = tk.StringVar()
loading_label = ttk.Label(main_frame, textvariable=loading_var,
                          font=("Segoe UI", 10, "italic"), bootstyle="secondary")
loading_label.grid(row=3, column=0, sticky="ewns", pady=5)
